...
export default function Footer() {
  ...
  return (
    <footer className="bg-slate-900 dark:bg-neutral-900 pt-5 pb-5 px-4 md:px-10 mt-10">
      <div className="max-w-7xl mx-auto">
        {/* Top Section: Logo & Nav */}
        <div className="flex justify-between items-center flex-col lg:flex-row flex-wrap gap-x-6 gap-y-8 text-center">
          <Link to={'/'} className="min-h-12 inline-block focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded">
            {/* logo */}
            <Logo color = 'invert' />
          </Link>

          <ul className="flex space-x-6 gap-y-4 max-lg:justify-center flex-wrap text-sm text-slate-400">
            {navLinks.map((link) => (
              <li key={link.label}>
                <Link to={link.route} className="hover:text-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded">
                  {link.label}
                </Link>
              </li>
            ))}
